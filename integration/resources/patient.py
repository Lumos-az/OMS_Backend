...
class setDefaultPatient(Resource):
    ...
    def post(self):
        ...
        print(args)
        try:
            search = PatientsJoinModel.query.filter(PatientsJoinModel.uid == args['uid'],
                                                    PatientsJoinModel.pid == args['pid'])
            if search.first() is None:
                res = pretty_result(code.DB_ERROR)
                res['Error'] = 'Not find the person'
                return res
            else:
                db.session.query(PatientsJoinModel).filter(PatientsJoinModel.uid == args['uid'],
                                                           PatientsJoinModel.isDefault == 1).update(
                    {PatientsJoinModel.isDefault: int(0)}, synchronize_session=False)
                db.session.commit()
                db.session.query(PatientsJoinModel).filter(PatientsJoinModel.uid == args['uid'],
                                                           PatientsJoinModel.pid == args['pid']).update(
                    {PatientsJoinModel.isDefault: int(1)}, synchronize_session=False)
                db.session.commit()
                return pretty_result(code.OK)
        except SQLAlchemyError as e:
            print(e)
    ...
    def post(self):
        try:
            ...
            if search.first() is None:
                ...
                db.session.query(RecordModel).filter(RecordModel.others == '待就诊',
                                                     RecordModel.rid == args['rid']).update({RecordModel.others: '已取消'},
                                                                                            synchronize_session=False)
                db.session.commit()
                db.session.query(BookingTimeModel).filter(BookingTimeModel.tid == tmp.timeid).update(
                    {BookingTimeModel.isFull: int(0), BookingTimeModel.state: (tmp.reservationNumber - 1)},
                    synchronize_session=False)
                db.session.commit()
                return pretty_result(code.OK)
        except SQLAlchemyError as e:
...
class newBooking(Resource):
    ...
    def post(self):
        try:
            ...
            args = self.parser.parse_args()
            # print(args)
            booking = BookingTimeModel.query.filter(BookingTimeModel.did == args['did'],
                                                    BookingTimeModel.date == args['Date'],
                                                    BookingTimeModel.time == args['time'])
...
class registerBydoctor(Resource):
    ...
    def post(self):
        try:
            ...
            HSrecord = DoctorModel.query.filter(DoctorModel.did == args['doctorid']).with_entities(DoctorModel.hid,
                                                                                                   DoctorModel.Depid).first()
            Date = BookingTimeModel.query.filter(BookingTimeModel.tid == args['timeid']).first()
            findresult = BookingTimeModel.query.join(RecordModel, RecordModel.timeid == BookingTimeModel.tid).filter(
                BookingTimeModel.date == Date.date, RecordModel.departmentid == HSrecord.Depid,
            ...
            if findresult.first() is None:
                ...
                if (time.first())[2] == 4:
                    ...
                    db.session.query(BookingTimeModel).filter(BookingTimeModel.tid == (time.first())[0]).update(
                        {BookingTimeModel.isFull: int(1), BookingTimeModel.state: int(4)},
                        synchronize_session=False)
                    db.session.commit()
                else:
                    db.session.query(BookingTimeModel).filter(BookingTimeModel.tid == (time.first())[0]).update(
                        {BookingTimeModel.state: int((time.first())[2] + 1)}, synchronize_session=False)
                    db.session.commit()
...
class registerByDepartment(Resource):
    ...
    def post(self):
        try:
            ...
            findresult = BookingTimeModel.query.join(RecordModel, RecordModel.timeid == BookingTimeModel.tid).filter(
                BookingTimeModel.date == args['date'], RecordModel.departmentid == args['depid'],
                RecordModel.hospitalid == args['hid'],
                RecordModel.identityNumber == args['pid'], RecordModel.others == '待就诊')

            if findresult.first() is None:
                # 根据时间，医院，部门获取医生的信息，返回book的编号，医生的id和医生的名字
                doctor = BookingTimeModel.query.join(DoctorModel, DoctorModel.did == BookingTimeModel.did).filter(
                    BookingTimeModel.time == args['time'], BookingTimeModel.date == args['date'],
                    DoctorModel.hid == args['hid'], DoctorModel.Depid == args['depid'],
                    BookingTimeModel.isFull == int(0)).with_entities(BookingTimeModel.tid, DoctorModel.did,
                                                                     DoctorModel.name, BookingTimeModel.state)
                ...
                if (doctor.first())[3] == 3:
                    db.session.query(BookingTimeModel).filter(BookingTimeModel.tid == (doctor.first())[0]).update(
                        {BookingTimeModel.isFull: int(1), BookingTimeModel.state: int(4)},
                        synchronize_session=False)
                    db.session.commit()
                else:
                    db.session.query(BookingTimeModel).filter(BookingTimeModel.tid == (doctor.first())[0]).update(
                        {BookingTimeModel.state: int((doctor.first())[3] + 1)}, synchronize_session=False)
                    db.session.commit()
                ...
                time = DoctorModel.query.filter(DoctorModel.hid == args['hid'],
                                                DoctorModel.Depid == args['depid']).join(BookingTimeModel,
                                                                                         BookingTimeModel.did == DoctorModel.did).with_entities(
                    BookingTimeModel.date, BookingTimeModel.time, BookingTimeModel.state,
