...
class Login(Resource):
    ...
    def get(self):
        ...
        try:
            ...
            user = UserModel.query.filter(UserModel.phoneNumber == args['username']).first()
            if len(args['username']) == 18:
                user = UserModel.query.filter(UserModel.uid == args['username']).first()
            #if判断，错误就退出，正确就继续
...
class ChangeCode(Resource):
    ...
    def post(self):
        ...
        try:
            ...
            if str.isalpha(args['newPassword']):
                return pretty_result(code.DB_ERROR,data="密码不能仅包含字母")
            elif str.isdigit(args['newPassword']):
                return pretty_result(code.DB_ERROR,data="密码不能仅包含数字")
